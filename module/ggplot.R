             ...
             fluidRow(
               ...
               box(width=3,status="success",
                   ...
                   dropdownButton(circle=FALSE, label=lang$t('参数设置'), br(),br(),
                                  fluidRow(
                                    column(width = 6,selectInput(ns('numeric'), lang$t("连续变量"), c(""),multiple = T )),
                                    column(width = 6,selectInput(ns('factor'), lang$t("分类变量"), c("") ,multiple = T )),
                   ...
                   if(T){
                     conditionalPanel(
                       condition = "input.plotType.includes('annotate')",ns = NS(id),
                       dropdownButton(circle=FALSE, label='annotate', br(),br(),
                                           fluidRow(
                                             column(width = 6,numericInput(ns("x_annotate"),       'x', value = 0 ) ),
                                             column(width = 6,numericInput(ns("y_annotate"),       'y', value = 0) ),
                                             column(width = 6,selectInput(ns("color_text"),    'color',   c("") ) ),
                                             column(width = 6,selectInput(ns("label_text"),    'label',   c("") ) ),
          ...
          if(!is.null(input$table ) ){
            ...
            observe({
              ...
              if(length(input$numeric)>1){
                df[,input$numeric] <- apply(df[,input$numeric],1,as.numeric)
              }
              ...
              if(length(input$factor)>1){
                df[,input$factor] <- apply(df[,input$factor],1,as.factor)
              }
              ...
              if( 'point'     %in% input$plotType){
                ...
                updateSelectInput(session, "x_point", label = 'x', choices = colnames(df) , 
                                  selected = input$x  )
                updateSelectInput(session, "group_point", label = 'group', choices = colnames(df) , 
                                  selected = '' )
              ...
              if( 'boxplot'   %in% input$plotType){
                ...
                updateSelectInput(session, "x_boxplot", label = 'x', choices = colnames(df) , 
                                  selected = input$x  )
                updateSelectInput(session, "group_boxplot", label = 'group', choices = colnames(df) , 
                                  selected = '' )
