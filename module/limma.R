               ...
               fluidRow(
                 ...
                 box(width = 3,status="success",
                     ...
                     dropdownButton(circle=FALSE, label=lang$t("参考数据"), status="success",icon = icon("download"),
                                    ...
                                    downloadBttn(ns("downloadSampleGroup"), lang$t("分组"), size='sm', block=TRUE) ),br(),
                     dropdownButton( label = lang$t("使用说明"), icon = icon('tachometer-alt'), circle = FALSE,br(),br(),
                                     h6("1、矩阵数据为标准化的数据。"),
                                     h6("2、点击“Show Data”可查看参考数据或输入的数据。"),
                                     h6("3、矩阵 中，ID 列为基因，列名“ID”不可改变。
                               其他列为样本计数，列名避免输入“-”、“。”等中文符号。"),
                                     h6("4、分组 中列名为小写“sample”与“condition”，sample 列对应矩阵样本列名，condition 列为分组情况。"),
                                     h6("5、确定参考数据无误后，设置分组及低表达过滤参考，点击“Analyze Data”。")
                     ) 
                 ) ) ),
      tabPanel(title = 'DEG',
               fluidRow(
                     ...
                     conditionalPanel(
                       ...
                       br(),dropdownButton(circle=FALSE, label=lang$t('参数设置'), br(),br(),
                                           ...
                                           conditionalPanel(
                                             ...
                                             numericInput(ns("heatmap_num"),label = lang$t("热图基因数"), value = 50) 
                                           ),
                                           conditionalPanel(
                                             condition = "input.plot=='volcano'",ns = NS(id),
                                             numericInput(ns("volcano_num"),label = lang$t("火山图标签数"), value = 10),
                                             selectInput(ns('theme'),lang$t('火山图主题'),selected = 'bw',choices =theme_select )
                                           )
                       )
                     ), br(),
         ...
         if( is.null(input$file1)  ){
           ...
           group <- reactive({
             df2 <- data.frame(
               ...
               condition= c( rep('con',3),rep('dex',3) )
             )
             return(df2 )
           })
         }
