               ...
               fluidRow(
                 ...
                 box(width = 3,status="success",
                     ...
                     dropdownButton( label = lang$t("使用说明"), icon = icon('tachometer-alt'), circle = FALSE,
                                     br(),br(),
                                     # choices = c('y ~ x','y ~ poly(x, 2)','y ~ log(x)', "y ~ s(x, bs = 'cs')"), 
                                     h6("1、参考数据中，列名均为小写，且需要保证一致。
                             x 为小写x，y为小写y，group为小写group，face为小写face。
                             group对应分组，face对应分面，列名不可改变，但对应列内容可自行编辑。"),
                                     h6("2、R 与 P仅为直线相关性，对应拟合为 lm, y~x ，其他拟合方式的时候，恐不具有代表性。"),
                                     h6("3、点线、填充颜色的“数量”需不小于 group列的分组数。"),
                                     h6("4、方程中的x与y的对应关系有:"),
                                     h6(" y ~ x "),
                                     h6("y ~ poly(x, n) "),
                                     h6("y ~ log(x)"),
                                     h6("等。其中 y ~ poly(x, n) 为多项式关系，n为项数，为1、2、3等整数，
                             如 y ~ poly(x, 2) 表示二项式。") 
                     )   ) )
               ),
      tabPanel(title = 'Plot',
