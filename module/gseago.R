               ...
               fluidRow(
                 ...
                 box(width = 3,status="success",
                     ...
                     fluidRow(
                       ...
                       column(width = 6, selectInput( inputId = ns("logFC_col"),   lang$t('logFC 列'),  c("")))
                     ),hr(),
                     downloadButton(outputId = ns("downloadSampleData"), lang$t("参考数据") ),br(),br(),
                     dropdownButton( label = lang$t("使用说明"), icon = icon('tachometer-alt'), circle = FALSE,br(),br(),
                                     h6("1、首先确定物种。"),
                                     h6("2、数据至少包含 ID、logFC 两列。"),
                                     h6("3、运行时间较长，点击运行后请耐心等待小会儿。")
                     )
                 ) )
               ),
      tabPanel(title = 'GSEA',
               fluidRow(
                 box(title=lang$t("GSEA 分析"),width=9,solidHeader=TRUE,status = "primary",background = "white",
                     splitLayout(cellWidths = c("100%"),plotOutput(ns("plot_gsea0"),height = 500 ) ) ),
                 box(width = 3,status="success",
